...
export async function POST(request: Request) {
  try {
    ...
    // Update is_locked to false
    await sql`
      UPDATE scores SET is_locked = false, updated_at = now()
      WHERE judge_id = ${judge_id} AND team_id = ${team_id}
    `;

    // Audit Log
    const afterData = { ...existing, is_locked: false };
    await sql`
      INSERT INTO score_audit (judge_id, team_id, action, actor, before_data, after_data, reason)
      VALUES (${judge_id}, ${team_id}, 'unlock', 'admin', ${JSON.stringify(existing)}, ${JSON.stringify(afterData)}, ${reason})
    `;

    return NextResponse.json({ success: true });
  } catch (error) {
    console.error(error);
    return NextResponse.json({ error: 'Failed to unlock score' }, { status: 500 });
  }
}
